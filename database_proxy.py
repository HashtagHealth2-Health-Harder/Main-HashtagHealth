...
def get_latest_tweet(): 
    global conn
    c = conn.cursor()
    tweet_table_id = c.execute("SELECT id FROM main ORDER BY DateTime DESC LIMIT 1").fetchone()
    tweet_info = c.execute("SELECT Tweet_Text from tweets WHERE id = (?)", (tweet_table_id)).fetchone()
    tweet_location = c.execute("SELECT Full_Name from region WHERE id = (?)", (tweet_table_id)).fetchone()
    tweet_bounding_box = c.execute("SELECT Region from users WHERE id = (?)", (tweet_table_id))
    return json.dumps({"info": tweet_info[0], "location": tweet_location[0]})
